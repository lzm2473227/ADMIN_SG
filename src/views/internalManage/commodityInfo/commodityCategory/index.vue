...
export default {
  ...
  methods:{
    ...
    async getdata() {
      ...
      let res = await this.$API.commodityInfo.classifyCatalog(data)
      if(res.code !== 'SUCCESS') return false
      let len = []
    ...
    async add() {
      if(!this.ruleForm.bid){
        ...
        let res = await this.$API.commodityInfo.classifyInsertClassify(data)
        if(res.code !== 'SUCCESS') return false
      ...
      if(this.ruleForm.bid){
        ...
        let res = await this.$API.commodityInfo.classifyUpdateClassify(data)
        if(res.code !== 'SUCCESS') return false
    ...
    async threedirectories() {
      ...
      let res = await this.$API.commodityInfo.classifyQueryCategoriesCatalog(data)
      console.log(res);
