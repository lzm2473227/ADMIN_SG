  ...
  <div class="tab">
    ...
    <div class="tab-body inside-table">
      ...
      >
        ...
        <el-table-column label="品牌编号" align="center" sortable width="150" show-overflow-tooltip>
          <template v-slot="scope">
            <span class="detail-info" @click="editproduct(scope.row, '2')">{{scope.row.code}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="brandName" label="品牌名称" sortable width="140" show-overflow-tooltip></el-table-column>
        <el-table-column label="品牌Logo" align="center" sortable width="110">
          <template v-slot="scope">
            <el-image class="preview-img" style="height: 20px" :src="scope.row.logo" @click="onPre(scope.row)"></el-image>
          </template>
        </el-table-column>
        <el-table-column prop="brandEnName" label="品牌名称(英文)" sortable width="170" show-overflow-tooltip></el-table-column>
        <el-table-column prop="countryOfOrigin" label="品牌原产地" sortable show-overflow-tooltip></el-table-column>
         <el-table-column label="商标注册证" align="center" sortable width="110">
          <template v-slot="scope">
            <el-image class="preview-img" style="height: 20px" :src="scope.row.brandLicense" @click="onPre(scope.row)"></el-image>
          </template>
        </el-table-column>
        <el-table-column prop="brandLicenseEffectiveDate" label="授权有效期" align="center" sortable width="160"></el-table-column>
        <el-table-column label="商标注册证" align="center" sortable width="110">
          <template v-slot="scope">
            <el-image class="preview-img" style="height: 20px" :src="scope.row.trademarkLicense" @click="onPre(scope.row)"></el-image>
          </template>
        </el-table-column>
        <el-table-column prop="trademarkLicenseEffectiveDate" label="商标有效期" align="center" sortable width="160"></el-table-column>
        <el-table-column prop="time" label="更新时间" align="center" sortable width="160"></el-table-column>
      </el-table>
    </div>
    <div class="bot">
...
export default {
  ...
  methods: {
      ...
      let res = await this.$API.commodityInfo.findCurrEnterpriseSellBrandInfoList(params)
        console.log(res);
          ...
          data.forEach((item, key) => {
            item.index = len[key]
            item.brandLicenseEffectiveDate = moment(item.brandLicenseEffectiveDate).format("YYYY-MM-DD HH:mm:ss");
            item.trademarkLicenseEffectiveDate = moment(item.trademarkLicenseEffectiveDate).format("YYYY-MM-DD HH:mm:ss");
          })
          this.tabledata = data
    },
    //删除单个机具
    ...
    editproduct(data) {
      this.$router.push({
        path: "/internalManage/commodityInfo/publishedBrandDetailInfo",
        query: {
          documentId: data.id,
        },
      });
