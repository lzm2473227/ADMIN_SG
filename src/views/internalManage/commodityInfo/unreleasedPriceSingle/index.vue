  ...
  <div class="tab">
    ...
    <div class="tab-body inside-table">
      ...
      >
        ...
        <el-table-column label="呈报人/部门" align="center" sortable width="160" >
          <template v-slot="scope">
            <span>{{scope.row.reportName}} / {{scope.row.reporterDept}}</span>
          </template>
        </el-table-column>
        <el-table-column label="最后经办人/部门" align="center"  sortable width="150" >
          <template v-slot="scope">
  ...
  methods: {
    ...
    onAdd(val, data) {
      if(val){
        ...
        this.$router.push({
          path: "/internalManage/commodityInfo/priceDetailInfoSingle",
          query: {
            documentId: documentId,
          },
        });
      } else{
        this.$router.push({path: "/internalManage/commodityInfo/priceDetailInfoSingle"});
      }
    },
    barcode(){},
    delproduct() {
