        ...
        <table border="1" class="">
          <tr>
            ...
            <td class="table-right" colspan="3">
              <input type="text" placeholder="请输入审批件标题" v-model="form.title" :disabled="isDisabled"/>
            </td>
          </tr>
          <tr>
...
export default {
  ...
  methods: {
    getData(){
      ...
      // if(sessionStorage.getItem('orderList')) this.baojiaList1 = JSON.parse(sessionStorage.getItem('orderList'))
    },
    async init(){
      ...
      if(this.documentId){
        ...
        if(this.radio2===1){
          ...
          this.baojiaList1 = this.serviceParam.inquirySheetList[0]
            this.xunjiaList1.forEach(row => {
              this.$refs.orderTable2.toggleRowSelection(row, true)
              row.xunjia = parseFloat(row.xunjia)
              row.quantity = parseFloat(row.quantity)
            })
            this.baojiaList1.forEach(row => {
              this.$refs.orderTable3.toggleRowSelection(row, true)
    ...
    async getAddFile(val) {
      this.serviceParam.inquirySheetList = []
      this.serviceParam.quotationSheetList = []
      ...
      //   isSubmit = true
      // }
      console.log(this.baojiaList1)
      this.baojiaList1.forEach(item => {
        item.baojia = parseFloat(item.baojia)
        item.subTotal = parseFloat(item.baojia)*parseFloat(item.quantity)
      })
      if(this.radio2 === 0) this.serviceParam.commArr = this.orderList
      this.serviceParam.inquirySheetList.push(this.xunjiaList1)
      this.serviceParam.quotationSheetList.push(this.baojiaList1)
      let params = {
        "creatorIdentityId": "",
        "creatorUserId": this.$store.state.user.registerUserId,
        "object": {
