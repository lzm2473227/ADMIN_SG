<template>
  <div class="tab">
    <div class="tab-title">
      <div class="left">
        <div class="print" @click="addStore" v-show="code === 'NATIONAL_DISTRIBUTOR'"><img class="icon" src="@/assets/images/add.png" alt=""><span class="axis">新增组织</span></div>
        <div class="print" @click="editstore()" v-show="code === 'NATIONAL_DISTRIBUTOR'"><img class="icon" src="@/assets/images/edit.png" alt=""><span class="axis">编辑组织</span></div>
        <div class="print" @click="delstore" v-show="code === 'NATIONAL_DISTRIBUTOR'"><img class="icon" src="@/assets/images/delete.png" alt=""><span class="axis">删除组织</span></div>
    ...
    <div class="tab-body inside-table">
      ...
      >
        ...
        <el-table-column label="机构代码" align="center" sortable width="200">
          <template v-slot="scope">
            <span class="detail-info" @click="editstore(scope.row)">{{scope.row.orgCode}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="name" label="组织名称" sortable width="220" show-overflow-tooltip></el-table-column>
        <el-table-column prop="parentName" label="上级组织" sortable width="220" show-overflow-tooltip></el-table-column>
        <el-table-column prop="address" label="联系地址" sortable show-overflow-tooltip></el-table-column>
        <el-table-column prop="remark" label="备注" sortable show-overflow-tooltip width="120"></el-table-column>
      </el-table>
    </div>
    <div class="bot">
    ...
    <div class="inp-bot search-set">
      <div class="search-ipt">
        <span class="ipt-left">组织名称</span>
        <input type="text" placeholder="请输入组织名称" v-model="ruleForm.storeName">
      </div>
      <!-- <div class="search-ipt">
...
import { queryList_Org, batchDelete_Org } from '@/api/orgSet'
...
export default {
  ...
  methods: {
      ...
      let params = {
        ...
        let res = await this.$API.orgSet.queryDtoList_Org(params)
        t.total= res.total;
   ...
   async delstore() {
    ...
    await this.$confirm('此操作将永久删除数据, 是否继续?', '提示', {
        ...
        this.$API.orgSet.batchDelete_Org(params).then(res => {
          if (res.code === "SUCCESS") {
            t.$message({
              message: "删除成功",
              type: "success",
    ...
    editstore(data) {
      ...
      if(data){
        ...
        if(this.multipleSelection.length != 1) return this.$message('请选择一个需要编辑的组织')
        id = this.multipleSelection[0].id
      }
      // console.log(data);
      this.$router.push({
        path: "/internalManage/orgSet/orgInfo",
        query: {
          id: id,
        },
      });
    ...
    addStore() {
      ...
      this.$router.push({ path: "/internalManage/orgSet/orgInfo" });
    },
