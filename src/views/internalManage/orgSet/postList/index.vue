<template>
  <div class="tab">
    <div class="tab-title">
      <div class="left">
        <div class="print" @click="addStore" v-show="code === 'NATIONAL_DISTRIBUTOR'"><img class="icon" src="@/assets/images/add.png" alt=""><span class="axis">新增岗位</span></div>
        <div class="print" @click="editstore()" v-show="code === 'NATIONAL_DISTRIBUTOR'"><img class="icon" src="@/assets/images/edit.png" alt=""><span class="axis">编辑岗位</span></div>
        <div class="print" @click="delstore" v-show="code === 'NATIONAL_DISTRIBUTOR'"><img class="icon" src="@/assets/images/delete.png" alt=""><span class="axis">删除岗位</span></div>
    ...
    <div class="tab-body inside-table">
      ...
      >
        ...
        <el-table-column label="岗位内部编码" align="center" sortable width="180">
          <template v-slot="scope">
            <span class="detail-info" @click="editstore(scope.row)">{{scope.row.privateCode}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="name" label="岗位名称" sortable width="150" show-overflow-tooltip></el-table-column>
        <el-table-column prop="deptName" label="部门名称" sortable width="150" show-overflow-tooltip></el-table-column>
        <el-table-column prop="orgName" label="所属组织" align="center" sortable width="150" show-overflow-tooltip></el-table-column>
        <el-table-column prop="remark" label="备注" align="center" sortable show-overflow-tooltip></el-table-column>
      </el-table>
    </div>
    <div class="bot">
    ...
    <div class="inp-bot search-set">
      <div class="search-ipt">
        <span class="ipt-left">岗位名称</span>
        <input type="text" placeholder="请输入岗位名称" v-model="ruleForm.storeName">
      </div>
      <!-- <div class="search-ipt">
...
import { findRoleList_Role } from '@/api/orgSet'
export default {
  name: "tab",
  components: {
    Page
  ...
  methods: {
    async getdata(){
      ...
      this.tabledata = []
      let res = await findRoleList_Role(params)
          t.total= res.total;
    ...
    delstore() {
      if(this.multipleSelection.length <= 0) return this.$message.error({message: '请选择需要删除的岗位'})
      this.$confirm('此操作将永久删除数据, 是否继续?', '提示', {
        ...
        this.$API.orgSet.batchDelete_Role(params)
    ...
    editstore(data) {
      ...
      if(data){
        ...
        if(this.multipleSelection.length != 1) return this.$message('请选择一个需要编辑的岗位')
        id = this.multipleSelection[0].id
      }
      this.$router.push({
        path: "/internalManage/orgSet/postInfo",
        query: {
          id: id,
        },
      });
    ...
    addStore() {
      this.$router.push({ path: "/internalManage/orgSet/postInfo"});
    },
