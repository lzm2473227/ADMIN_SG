  ...
  <div class="tab">
    ...
    <div class="tab-body inside-table">
      ...
      >
        ...
        <el-table-column label="编码计划编号" show-overflow-tooltip align="center" sortable width="170">
          <template v-slot="scope">
            <span class="detail-info" @click="onAdd(scope.row)">{{scope.row.codeNumber}}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="specsParameter" show-overflow-tooltip label="产品规格" sortable></el-table-column>
        <el-table-column prop="brandName" show-overflow-tooltip label="产品品牌" sortable></el-table-column>
        <el-table-column prop="createTime" label="编码计划时间" align="center" sortable width="170"></el-table-column>
        <el-table-column label="编码明细" align="center" sortable width="160">
            <template v-slot="scope">
              <span class="detail-info" @click="onDetail(scope.row)">查看</span>
            </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    async getdata() {
      ...
      let res = await this.$API.code.codeInfoPlanList(params)
        console.log(res);
          let data = res.data;
    ...
    onAdd(data) {
      this.$router.push({
        path: "/produce/codingScheme/codeListInfo",
        query: {
    ...
    onDetail(data){
      this.$router.push({
        path: "/produce/codingScheme/codeListDetail",
        query: {
          documentId: data.documentId,
        },
      })
