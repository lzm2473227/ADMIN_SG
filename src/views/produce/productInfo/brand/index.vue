<template>
  <div class="tab">
    <div class="tab-title">
      <div class="left">
        <div class="print" @click="onAddBrand"><img class="icon" src="@/assets/images/add.png" alt=""><span class="axis">新增品牌</span></div>
        <div class="print" @click="edit('1')" ><img class="icon" src="@/assets/images/edit.png" alt=""><span class="axis">编辑品牌</span></div>
        <div class="print" @click="del()"><img class="icon" src="@/assets/images/delete.png" alt=""><span class="axis">删除品牌</span></div>
    ...
    <div class="tab-body inside-table">
      ...
      >
        ...
        <el-table-column label="品牌编号" align="center" sortable width="190">
          <template v-slot="scope">
            <span class="detail-info" @click="edit('2', scope.row.id)">{{scope.row.code}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="brandEnName" label="品牌名称" sortable></el-table-column>
        <el-table-column label="品牌图片" align="center" sortable width="160">
          <template v-slot="scope">
          ...
          <form action="">
            <table border="1" class="" style="width: 100%; font-size: 14px;">
              <tr>
                ...
                <td class="table-right">
                  <input v-show="isEdit === '1'" type="text" placeholder="请输入品牌名称 (中文)" v-model="ruleForm.brandName" />
                  <span v-show="isEdit === '2'">{{ruleForm.brandName}}</span>
                </td>
              </tr>
              <tr>
                <td class="table-left">品牌名称 (英文)</td>
                <td class="table-right">
                  <input v-show="isEdit === '1'" type="text" placeholder="请输入品牌名称 (英文)" v-model="ruleForm.brandEnName" />
                  <span v-show="isEdit === '2'">{{ruleForm.brandEnName}}</span>
                </td>
                <td class="table-left">备注事项</td>
                <td class="table-right">
                  <input v-show="isEdit === '1'" type="text" placeholder="请输入备注事项" v-model="ruleForm.remark" />
                  <span v-show="isEdit === '2'">{{ruleForm.remark}}</span>
                </td>
              </tr>
              <tr>
              ...
              <tr>
                ...
                <td class="table-right" colspan="3" v-show="isEdit === '1'">
                ...
                <td class="table-right" colspan="3" v-show="isEdit === '2'">
...
export default {
  ...
  methods:{
     del(){
      if(this.multipleSelection.length <= 1) return this.$message('请选择一个需要删除的品牌')
      var data = {
        "object": {
      ...
      let res = await this.$API.commodityInfo.classifyDeleteClassify(data)
    ...
    async getdata() {
      ...
      let res = await this.$API.code.findCurrEnterpriseBrandInfoList(data)
      if(res.code !== 'SUCCESS') return false
      let len = []
