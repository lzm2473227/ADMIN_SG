  ...
  <div class="tab">
    ...
    <div class="tab-body inside-table">
      ...
      >
        ...
        <el-table-column label="活动编号" show-overflow-tooltip align="center" sortable width="170">
          <template v-slot="scope">
            <span class="detail-info" @click="onAdd(scope.row)">{{scope.row.activityNumber}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="activityName" show-overflow-tooltip label="活动名称" sortable width="220"></el-table-column>
        <el-table-column prop="name" label="活动位置" show-overflow-tooltip align="center" sortable width="150"></el-table-column>
        <el-table-column prop="activityDescribe" label="活动简述" sortable show-overflow-tooltip></el-table-column>
        ...
        <el-table-column label="有效期" sortable width="180" show-overflow-tooltip align="center" >
          <template v-slot="scope">
            <span>起{{scope.row.startTime}}</span><br>
            <span>至{{scope.row.endTime}}</span>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    async getdata() {
      ...
      let res = await this.$API.activity.queryActivityList(params)
        console.log(res);
          let data = res.data;
    ...
    onAdd(data){
      this.$router.push({
        path: "/promotion/activity/publishedActivityInfo",
        query: {
          id: data.id,
        },
      })
