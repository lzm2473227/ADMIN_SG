<template>
  <div class="tab">
    <div class="tab-title">
      <div class="left">
        <div class="print" @click="onLine()"><img class="icon" src="@/assets/images/add.png" alt=""><span class="axis">添加商品</span></div>
        <div class="print" @click="delOrder"><img class="icon" src="@/assets/images/delete.png" alt=""><span class="axis">删除商品</span></div>
        <div class="print" @click="onOrder()"><img class="icon" src="@/assets/images/add.png" alt=""><span class="axis">形成订单审批件</span></div>
        <div class="print" @click="onAdd()"><img class="icon" src="@/assets/images/add.png" alt=""><span class="axis">订单未审核批件列表</span></div>
    ...
    <div class="tab-body inside-table">
      ...
      >
        ...
        <el-table-column label="产品条形码" show-overflow-tooltip align="center" sortable width="140">
          <template v-slot="scope">
            <span class="detail-info" @click="onAdd('1', scope.row)">{{scope.row.barcode}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="commodityName" show-overflow-tooltip label="商品名称" sortable></el-table-column>
        <el-table-column prop="specsParameter" label="产品规格" show-overflow-tooltip align="center" sortable width="160"></el-table-column>
        <el-table-column prop="brandName" label="产品品牌" sortable show-overflow-tooltip width="160"></el-table-column>
        <el-table-column prop="storeName" label="入库数量" sortable show-overflow-tooltip align="center" width="120"></el-table-column>
        <el-table-column prop="retailPrice" label="包装比例" sortable align="center" width="120"></el-table-column>
        <el-table-column prop="tradePrice" label="已扫描数(箱)" sortable align="center" width="120"></el-table-column>
        <el-table-column prop="quantity" label="已扫描数(单品)" sortable align="center" width="140"></el-table-column>
      </el-table>
    </div>
    <div class="bot">
...
export default {
  ...
  methods: {
    async getdata() {
      ...
      let res = await this.$API.order.orderCartList(params)
        console.log(res);
          let data = res.data;
    ...
    onOrder(){
      if(this.multipleSelection.length <= 0) return this.$message('请选择需要形成的入库审批件')
      let storeIdArr = []
    ...
    //   if(!elementsAreEqual(storeIdArr)) return this.$message.error('请选择同一个店铺的商品')
      ...
      this.$router.push({
        path: "/stock/passToStock/unreleasedStock",
        query: {
          storeName: storeName,
          storeId: this.multipleSelection[0].storeId,
          enterpriseName: this.multipleSelection[0].orgName,
    ...
    onAdd(val, data) {
      if(val){
        ...
        this.$router.push({
          path: "/purchase/purchaseOrder/unapprovedApproval",
          query: {
            documentId: documentId,
          },
        });
      } else{
        this.$router.push({path: "/purchase/purchaseOrder/unapprovedApproval"});
      }
    },
    // 机具图片预览
    onPre(data){
