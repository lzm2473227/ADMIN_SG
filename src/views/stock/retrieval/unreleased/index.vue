<template>
  <div class="tab">
    <div class="tab-title">
      <div class="left">
        <div class="print" @click="onOrder()"><img class="icon" src="@/assets/images/add.png" alt=""><span class="axis">形成出库单审批件</span></div>
        <div class="print" @click="onAdd()"><img class="icon" src="@/assets/images/add.png" alt=""><span class="axis">出库单未审核批件列表</span></div>
    ...
    <div class="tab-body inside-table">
      ...
      >
        ...
        <el-table-column label="关联单据" show-overflow-tooltip align="center" sortable width="140">
          <template v-slot="scope">
            <span class="detail-info" @click="onOrderDetail(scope.row)">{{scope.row.dealNumber}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="commodityName" show-overflow-tooltip label="商品内容" sortable min-width="200"></el-table-column>
        <el-table-column prop="totalQuantity" label="数量" sortable show-overflow-tooltip align="center" width="100"></el-table-column>
        <el-table-column prop="penterpriseName" label="采购方" show-overflow-tooltip sortable width="200"></el-table-column>
        <el-table-column prop="storeName" label="店铺名称" sortable show-overflow-tooltip width="120"></el-table-column>
        <el-table-column prop="retailPrice" label="出库扫描状态" sortable show-overflow-tooltip align="center" width="130"></el-table-column>
        <el-table-column prop="receivingUnit" label="收货单位" sortable show-overflow-tooltip width="200"></el-table-column>
        <el-table-column prop="consignee" label="收货联系人" sortable show-overflow-tooltip align="center" width="120"></el-table-column>
        <el-table-column prop="telephone" label="联系号码" sortable align="center" width="120"></el-table-column>
        <el-table-column prop="address" label="收货人地址" sortable show-overflow-tooltip width="220"></el-table-column>
      </el-table>
    </div>
    <div class="bot">
...
export default {
  ...
  methods: {
    ...
    onOrder(data){
      if(this.multipleSelection.length <= 0) return this.$message('请选择需要形成的出库审批件')
      // let storeIdArr = []
    ...
    //   if(!elementsAreEqual(storeIdArr)) return this.$message.error('请选择同一个店铺的商品')
      // sessionStorage.setItem('stockList', JSON.stringify(this.multipleSelection))
      // let storeName = this.multipleSelection[0].storeName
      this.$router.push({
        path: "/stock/retrieval/unreleasedRetrievalInfo",
        query: {
          dealNumber: this.multipleSelection[0].dealNumber,
          oddComm: this.multipleSelection[0].commodityName
        },
      })
    },
    onOrderDetail(data){
      this.$router.push({
        path: "/stock/retrieval/orderDetail",
        query: {
    ...
    onAdd() {
      this.$router.push("/stock/retrieval/unreleasedList")
    },
    // 机具图片预览
    onPre(data){
      this.srcList = []
