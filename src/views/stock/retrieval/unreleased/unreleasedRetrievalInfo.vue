  ...
  <div class="tab">
    ...
    <div class="table-main">
      <form action="" class="info-form">
        ...
        <table border="1" class="">
          <tr>
            ...
            <td class="table-right" colspan="3">
              <input type="text" placeholder="关于xxx的出库单审批" v-model="form.title" :disabled="isDisabled"/>
            </td>
          </tr>
          <tr>
        ...
        <table border="1" class="">
          <tr>
            <td class="table-left">出库单号</td>
            <td class="table-right" style="width: 305px;">
              <input type="text" placeholder="系统分配" v-model="serviceParam.dealNumber" disabled/>
            </td>
            <td class="table-left">出库说明</td>
            <td class="table-right" style="width: 305px;">
              <input type="text" placeholder="请输入出库说明" v-model="serviceParam.explains" :disabled="isDisabled"/>
            </td>
          </tr>
          <tr>
            <td class="table-left">出库仓库</td>
            <td class="table-right" style="width: 305px;position: relative;">
              <span>{{stock.warehouseName}}</span>
              <span @click="onStack" class="choose-sample">选择仓库</span>
            </td>
            <td class="table-left">出库时间</td>
            <td class="table-right" style="width: 305px;">
              <span></span>
            </td>
          </tr>
          <tr>
            <td class="table-left" style="border-left: none;">仓库地址</td>
            <td class="table-right" colspan="3">
              <span>{{stock.province}}{{stock.city}}{{stock.county}}{{stock.address}}</span>
            </td>
          </tr>
          <tr>
            <td class="table-left">出库操作人</td>
            <td class="table-right" style="width: 305px;">
              <span>{{stock.contacts}}</span>
            </td>
            <td class="table-left">备注事项</td>
            <td class="table-right" style="width: 305px;">
              <input type="text" placeholder="请输入备注事项" v-model="serviceParam.remarks" :disabled="isDisabled"/>
            </td>
          </tr>
          <tr>
          ...
          <tr>
            <td class="table-left" style="border-left: none;">收货单位</td>
            <td class="table-right" colspan="3">
              <input type="text" placeholder="请输入详细地址" v-model="serviceParam.receivingUnit" :disabled="isDisabled" />
            </td>
          </tr>
          <tr>
            <td class="table-left" style="border-left: none;">收货人所在地</td>
          ...
          <tr>
            ...
            <td class="table-left">联系人手机号码</td>
            <td class="table-right" style="width: 305px;">
          ...
          <tr>
            ...
            <td class="table-right" style="width: 305px;">
              <span>{{serviceParam.dealNumber}}</span>
            </td>
          ...
          <tr>
          <tr style="vertical-align: top">
            <td class="table-left" style="padding-top: 12px">出库明细</td>
            <td class="table-right" colspan="3">
              <!-- <div class="addComm">
              ...
              >
                  ...
                  <el-table-column prop="quantity" label="待出库数" width="120" align="center" sortable show-overflow-tooltip></el-table-column>
                  <el-table-column prop="quantity" label="出库数量"  width="120" align="center" sortable show-overflow-tooltip></el-table-column>
                  <el-table-column prop="bili" label="包装比例"  width="120" align="center" sortable show-overflow-tooltip></el-table-column>
                  <el-table-column label="已扫描数(箱)"  width="120" align="center" sortable show-overflow-tooltip>
                    <template v-slot="scope">
                      <span>{{scope.row.xiang}}</span>
                    </template>
                  </el-table-column>
                  <el-table-column prop="num" label="已扫描数(单品)"  width="140" align="center" sortable show-overflow-tooltip></el-table-column>
              </el-table>
            </td>
          </tr>
        </table>
      ...
      <div class="dialog-content" style="width: 620px">
        ...
        <div class="table-main">
          ...
          >
            ...
            <el-table-column prop="warehouseName" label="仓库名称" align="center" show-overflow-tooltip width="140"></el-table-column>
            <el-table-column prop="address" label="仓库地址" show-overflow-tooltip></el-table-column>
          </el-table>
  ...
  methods: {
    async init(){
      ...
      if(this.documentId){
        ...
        // 获取仓库详情
        this.getStockInfo(this.serviceParam.warehouseId)
      }
      // 获取订单详情
    ...
    async getOrderDetail(val){
      let orderId = ''
      if(this.documentId) orderId = this.serviceParam.dealNumber
      if(this.$route.query.dealNumber){
        orderId = this.$route.query.dealNumber
        this.serviceParam.dealNumber = this.$route.query.dealNumber
      }
      let params = {
        "object": {
          "dealNumber": orderId,
      ...
      if(this.$route.query.dealNumber){
        this.serviceParam.storeName = res.data.storeName
        this.serviceParam.penterpriseName = res.data.penterpriseName
        this.serviceParam.receivingUnit = res.data.receivingUnit
        this.serviceParam.telephone = res.data.telephone
        this.serviceParam.consignee = res.data.consignee
        this.serviceParam.address = res.data.address
        this.serviceParam.province = res.data.province
        this.serviceParam.city = res.data.city
        this.serviceParam.county = res.data.county
        this.selectedOptions = TextToCode[res.data.province][res.data.city][res.data.county].code
        this.serviceParam.dealNumber = this.$route.query.dealNumber
      }
      this.stockList = res.data.orderCommodityListItems
      this.$nextTick(() => {
        this.stockList.forEach((row,index) => {
    ...
    async getStockInfo(val){
      let params = {
        "object": {
          "warehouseId": val
        }
      } 
      let res = await this.$API.stock.warehouseDetail(params)
      if(res.code === 'SUCCESS'){
        this.stock = res.data
      }else{
        return false
    ...
    async getAddFile(val) {
      ...
      let state = 0
      let isSubmit = false
      let documentType = this.$route.query.storeId ? 'CKBJ' : 'CKSP'
      ...
      if(!this.stock.warehouseName) return this.$message.error('请选择出库仓库')
      let commIdArr = []
      this.multipleSelection.forEach(item => {
        commIdArr.push(item.id)
      })
      let serviceParam = this.serviceParam
      serviceParam.oddComm = this.$route.query.oddComm
      serviceParam.warehouseId = this.stock.id
      serviceParam.stockList = this.stockList
      let params = {
        "creatorIdentityId": "",
    ...
    getCommodityList() {
      ...
      this.$API.stock.warehouseList(params).then(res => {
          let data = res.data
    ...
    onSure(){
      this.stock = this.multipleSelection4[0]
      this.isShowComm = false
    },
    ...
    onQuoteDetail(){
      this.$router.push({
        path: "/purchase/purchaseOrder/quoteDetail",
        query: {
